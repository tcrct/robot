...
import com.robot.agv.vehicle.exchange.commands.SendRequestCommand;
import com.robot.agv.vehicle.exchange.commands.SetPeriodicStateRequestEnabledCommand;
import com.robot.agv.vehicle.exchange.commands.SetStateRequestIntervalCommand;
...
public class StatusPanel
   ...
   */
  @Inject
  public StatusPanel(@Assisted RobotProcessModelTO processModel,
  ...
  public void processModelChange(String attributeChanged, VehicleProcessModelTO newProcessModel) {
    ...
    if (Objects.equals(attributeChanged,
      ...
      updateCommAdapterConnected(processModel.isCommAdapterConnected());
            ...
            RobotProcessModel.Attribute.CURRENT_STATE.name())) {
      updateCurrentState(processModel.getCurrentState());
    }
    else if (Objects.equals(attributeChanged,
            RobotProcessModel.Attribute.PERIODIC_STATE_REQUESTS_ENABLED.name())) {
  ...
  private void initComponents() {
    ...
    ResourceBundle bundle = ResourceBundle.getBundle("config/commadapter/Bundle"); // NOI18N
  ...
  private void buttonGetStateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonGetStateActionPerformed
    SendRequestCommand command = new SendRequestCommand(new OrderRequest(new Protocol.Builder().build()));
